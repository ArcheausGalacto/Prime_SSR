...
# raise the limit to 1 000 000 digits:
...
def parse_sci(s: str) -> int:
    ...
    Parse strings of the form
      ...
      1e+102
      1.234e5000
    into an exact Python int, without ever converting to float.
    """
    ...
    if 'e' in s:
        ...
        # handle a decimal point in the coefficient
        if '.' in coeff_str:
            int_part, frac_part = coeff_str.split('.', 1)
        ...
        if exp >= 0:
            ...
            # negative exponent → integer division
            return coeff // (10**(-exp))
    else:
        return int(s)
...
def find_best_prime(center: int,
                    ...
                    ab_table: np.ndarray,
                    delta0: int):
    # only search x from 0 up to +b_range
    lo, hi = 0, b_range

    # first x ≥ 0 with x ≡ δ₀ (mod 6)
    start = lo + ((delta0 - lo) % 6)
    xs = list(range(start, hi+1, 6))

    # evaluate ab_table at x (no abs needed)
...
def main():
    ...
    print(f"Searching for best prime in [{sci_pow_plus(center)} … {sci_pow_plus(center + b_range)}]")
