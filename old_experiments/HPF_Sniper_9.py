...
def next_prime(n: int) -> int:
    # simple wheel (+2,/+4) stepping from nearest odd
...
# ─────────────────────────────────────────────────────────────────────────────
# 3) √k-enhanced oscillator fit (from 9000-range refinement)
# ─────────────────────────────────────────────────────────────────────────────
ω = 2 * math.pi / 6
...
def compute_delta0() -> int:
    φ = 2.1382  # original fit phase
    δ = (1 / ω) * (1.5 * math.pi - φ)
    return int(round(δ)) % 6

# ─────────────────────────────────────────────────────────────────────────────
# 4) FFT build of ab_table[x] = # of unordered ways to write x = p1+p2
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# 5) Pick and verify the best offset k
# ─────────────────────────────────────────────────────────────────────────────
def find_candidate(center: int,
    ...
    # 5a) Try largest-k local minima of f(k) with ab[k]>0
    local = [
        ks[i]
    ...
    if local:
        for k in sorted(local, reverse=True):
            cand = center + k
            if fast_isprime(cand):
                return k, cand, counts[ks.index(k)]

    # 5b) Fallback: survivors with globally minimal ab[k]>0, try smallest k first
    ...
    # 5c) Ultimate fallback: next prime above center
